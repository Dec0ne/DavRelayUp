...
import (
	"GoRelayServer/lib/Azure/go-ntlmssp"
	"bufio"
	"bytes"
	"crypto/md5"
	"encoding/base64"
...
func (req *NTLMBindRequest) appendTo(envelope *ber.Packet) error {
	request := ber.Encode(ber.ClassApplication, ber.TypeConstructed, ApplicationBindRequest, nil, "Bind Request")
	request.AppendChild(ber.NewInteger(ber.ClassUniversal, ber.TypePrimitive, ber.TagInteger, 3, "Version"))
	request.AppendChild(ber.NewString(ber.ClassUniversal, ber.TypePrimitive, ber.TagOctetString, "NTLM", "User Name"))

	// generate an NTLMSSP Negotiation message for the  specified domain (it can be blank)
	//negMessage, err := ntlmssp.NewNegotiateMessage("SEVENKINGDOMS", "WINTERFELL")
	negMessage, err := ntlmssp.NewNegotiateMessage("", "")
	if err != nil {
		return fmt.Errorf("err creating negmessage: %s", err)
	}

	// append the generated NTLMSSP message as a TagEnumerated BER value
	if len(req.NTLMSSPType1) > 0 {
		negMessage = req.NTLMSSPType1
	}
...
// NTLMChallengeBind performs the NTLMSSP bind operation defined in the given request
func (l *Conn) CustomNTLMChallengeBind(ntlmBindRequest *NTLMBindRequest) (*NTLMBindResult, error) {
	if !ntlmBindRequest.AllowEmptyPassword && ntlmBindRequest.Password == "" && ntlmBindRequest.Hash == "" {
		return nil, NewError(ErrorEmptyPassword, errors.New("ldap: empty password not allowed by the client"))
	}
	...
	if len(packet.Children) == 2 {
		if len(packet.Children[1].Children) == 3 {
			...
			l.Debug.Printf("%d: found ntlmssp challenge: %s", msgCtx.id)

			fmt.Printf("---------------------------\n%d: found ntlmssp challenge: %s\n---------------------------\n", msgCtx.id, enchex.EncodeToString(ntlmsspChallenge[24:32]))

			fmt.Println(base64.StdEncoding.EncodeToString(ntlmsspChallenge))

			in := bufio.NewReader(os.Stdin)
			if _, err := fmt.Fscan(in, &ntlm3); err != nil {
				log.Fatal(err)
			}
	...
	if ntlmsspChallenge != nil {
		...
		if ntlm3 != "" {
			responseMessage, _ = base64.StdEncoding.DecodeString(ntlm3)
			//copy(responseMessage[135:139], []byte{0x05, 0x02, 0x89, 0xa0})
		}
...
type RelayResult struct {
	...
	NTLMSSP_NEGOTIATE *NTLMBindRequest
	NTLMSSP_CHALLENGE []byte
	NTLMSSP_AUTH      []byte
}

func (l *Conn) GetNTLMChallenge(ntlmBindRequest *NTLMBindRequest, NTLMHASH ...[]byte) (*RelayResult, error) {
	// Send NTLMSSP Request Type 1 (NTLMSSP_NEGOTIATE)
	msgCtx, err := l.doRequest(ntlmBindRequest)
	if err != nil {
		return nil, err
	}
	...
	// Read NTLMSSP Response Type 2 (NTLMSSP_CHALLENGE)
	packet, err := l.readPacket(msgCtx)
	if err != nil {
		return nil, err
	}
	...
	if len(packet.Children) == 2 {
		if len(packet.Children[1].Children) == 3 {
			...
			result.NTLMSSP_CHALLENGE = ntlmsspChallenge
		}
	}
...
func (l *Conn) SendNTLMResponse(result *RelayResult) (*RelayResult, error) {
	var err error
	packet := ber.Encode(ber.ClassUniversal, ber.TypeConstructed, ber.TagSequence, nil, "LDAP Request")
	packet.AppendChild(ber.NewInteger(ber.ClassUniversal, ber.TypePrimitive, ber.TagInteger, l.nextMessageID(), "MessageID"))
	...
	// append the challenge response message as a TagEmbeddedPDV BER value
	auth := ber.Encode(ber.ClassContext, ber.TypePrimitive, ber.TagEmbeddedPDV, result.NTLMSSP_AUTH, "authentication")
	request.AppendChild(auth)
	packet.AppendChild(request)

	result.MsgCtx, err = l.sendMessage(packet)
	if err != nil {
		return nil, fmt.Errorf("send message: %s", err)
	}
	defer l.finishMessage(result.MsgCtx)
	packetResponse, ok := <-result.MsgCtx.responses
	if !ok {
		return nil, NewError(ErrorNetwork, errors.New("ldap: response channel closed"))
	}
	packet, err = packetResponse.ReadPacket()
	l.Debug.Printf("%d: got response %p", result.MsgCtx.id, packet)
	if err != nil {
		return nil, fmt.Errorf("read packet: %s", err)
	}
	return result, GetLDAPError(packet)
